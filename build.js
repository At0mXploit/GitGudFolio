...
async function buildCommand(username, program) {
  ...
  const opts = {
    ...
    limit: program.limit ? parseInt(program.limit, 10) : null,
    repos: program.repos
      ? program.repos.split(",").map(r => r.trim()).filter(Boolean)
      : null,
    customSocials: (program.social || []).map(s => {
      const idx = s.indexOf(":");
      if (idx < 0) return null;
      return { label: s.slice(0, idx).trim(), url: s.slice(idx + 1).trim() };
    }).filter(Boolean),
    friends: (program.friend || []).map(f => {
      const parts = f.split("|");
      return {
        name: (parts[0] || "").trim(),
        url: (parts[1] || "").trim(),
