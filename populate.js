...
function getSocialIcon(label) {
  ...
  return SOCIAL_ICONS[key] || "fas fa-link";
}
        ...
        try {
          ...
          document.getElementById("about").innerHTML = `
            ...
            user.location
                ...
                ${customSocials.map(({ label, url }) => {
                  const icon = getSocialIcon(label);
                  const display = label.charAt(0).toUpperCase() + label.slice(1);
                  return `<span><a href="${url}" target="_blank" class="socials"><i class="${icon}"></i>&nbsp;${display}</a></span>`;
                }).join("\n                ")}
                </div>
                `;
